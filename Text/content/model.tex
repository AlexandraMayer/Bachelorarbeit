...
Im dem von mir erstellten Fahrgastwechsel-Modell modelliere ich das Verhalten der Personen auf dem Bahnsteig und im Zug nicht, sondern nur den Fahrgastwechsel an sich. Soll in Zukunft auch das Verhalten im Inneren des Zuges und am Bahnsteig simuliert werden, kann mein Modell mit anderen bereits existierenden kombiniert werden. Für das Verhalten Einsteiger und Platzmacher nach dem Einsteigen kann das Modell der Sitzplatzwahl verwendet werden, dass Jakob Schöttl in seiner Masterarbeit an der Hochschule München erstellt hat (\cite{Schottl.2016}). Nachdem Aussteiger den Zug verlassen haben, kann zudem das bereits von crowd:it verwendete "`Optimal Steps"'-Modell für den weiteren Pfad der Aussteiger verwendet werden.
\subsection{Anforderungen an das Modell}
Das Fahrgastwechsel-Modell stellt den Entscheidungsprozess der Fahrgäste dar. Dabei sollen die Verhaltensweisen "`früher Einsteigen"' und "`im Weg Stehen"' modelliert werden, da diese an über 40 \% der Türen auftreten, und damit signifikant oft. Das Verhalten "`früher Einsteigen"' beschreibt dabei, dass eine Person beginnt in den Zug einzusteigen, bevor der letzte Aussteiger den Zug verlassen hat. Das Verhalten "`im Weg Stehen"' bedeutet, dass eine Person in einem Bereich der Tür stehen, in dem andere Personen ein- und aussteigen, und diese somit einen Bogen um die Person machen müssen.\\
Zudem sollte das Modell in der Lage sein, das Merkmal "`sperrig"' einer Person darstellen zu können. Dieses Merkmal beschreibt, dass eine Person mehr Platz einnimmt als andere Personen, da sie \zB ein Gepäckstück mit sich tragen. Dieses Merkmal wurde ebenfalls an über 40 \% der gefilmten Türen beobachtet und besitzt zudem einen zeitlichen Einfluss. Wird das Merkmal modelliert, so sollte es zu einer Verlangsamung des Fahrgastwechsels führen. \\
Neben den genannten Verhaltensweisen und dem Merkmal sollte ein Fahr\-gast\-wech\-sel-Mo\-dell darstellen können, dass Personen zu unterschiedlichen Zeitpunkten mit dem Aussteigen beginnen. Nach der Türöffnung warten sie unterschiedlich lange, bis sie den ersten Fuß auf den Bahnsteig setzten. Zudem gibt es Personen, die den Türbereich erst betreten, wenn sich in diesem keine andere Person mehr befindet. Auch dieses Verhalten sollte im Modell modelliert werden. \\
Das Modell sollte zudem abbilden wie Personen einen Platz zum Warten suchen. Sind im Zug Aussteiger vorhanden, so warten Personen, die an dieser Tür einsteigen wollen, bis sich die Möglichkeit bietet einzusteigen. Dabei können diese Personen in sogenannten "`Wartebereichen"' rechts und links der Tür stehen, oder sich "`im Weg"' der anderen positionieren. Wo sich Personen positionieren, um zu warten bis sie einsteigen können, sollte im Modell modelliert werden.
...
\begin{figure}[H]
	\centering
		\includegraphics[angle=270, width=0.38\textwidth]{pictures/model/kognitive_heuristic_model/alight_sketch.png}
...
\begin{figure}[H]
	\centering
		\includegraphics[angle=270, width=0.42\textwidth]{pictures/model/kognitive_heuristic_model/boarding_sketch.png}
...
Platzmacher starten im Wagon und gehen zunächst auch in die Wartebereiche neben den Türen, danach ist ihr Ziel dasselbe wie bei den Einsteigern, siehe \figurename \ref{fig:SkizzePlatzmacher}. Der Zwischenzielbereich der Platzmacher, ist in hellgrün eingezeichnet. Zu diesem Zwischenziel kann auch, der in lila dargestellte Bereich, der "`Im Weg"'-Bereich werden, wenn eine Person aufgrund der Heuristik im Weg steht. Der Zielbereich der Platzmacher ist in dunkelgrün dargestellt.
\begin{figure}[H]
	\centering
		\includegraphics[angle=270, width=0.42\textwidth]{pictures/model/kognitive_heuristic_model/spacemaker_sketch.png}
...
Liegt auf dem Weg ein Hindernis, wird versucht eine Person der gefolgt werden kann, einen Leader, zu finden. Wird kein Leader gefunden, wird der Algorithmus \ref{alg:SidewaysEvasion} der "`Sideways Evasion"'-Heuristik verwendet, um den nächsten Schritt zu finden.

Für die Abfrage bei defensiven Typen wird dabei der Algorithmus \ref{alg:StepPossible} verwendet, um zu überprüfen ob die defensive Person einen Schritt gehen kann. 
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.7\textwidth]{pictures/model/algorithm/heuristics/sideways_evasion_heuristic.png}
...
\end{figure}  Der Algorithmus für die "`Sideways evasion"'-Heuristik mit meiner Anpassung für defensive Typen ist in Algorithmus \ref{alg:SidewaysEvasion} dargestellt.
...
\begin{algorithm} [H]
	...
	\SidewaysEvasionHeuristic{} {
		...
		\If{zurückgegebene Position $pos_t$ ist $pos_{current}$} {
			...
			Position $pos_c$ hat die kleinere Distanz zum Ziel \\
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.65\textwidth]{pictures/model/algorithm/heuristics/tangential_evasion_heuristic.png}
...
Neben den vorgestellten Heuristiken mit der Anpassung für defensive Agenten, definiere ich zudem noch die "`aggressive Follower"'-Heuristik. 
\begin{figure}[H]
	\centering
		\includegraphics[width=0.65\textwidth]{pictures/model/algorithm/heuristics/aggressive_follower_heuristic.png}
	\caption{Prozessdiagramm der "`aggressiven Follower"'-Heuristik. Quelle: Eigene Darstellung.}
	\label{fig:AFH}
\end{figure}Diese soll für aggressive Typen eingesetzt werden, da ich davon ausgehe, dass diese Agenten zunächst versuchen die "`Sideways evasion"'-Heuristik zu verwenden, bevor sie einer Person folgen.
Bei der "`aggressiven Follower"'-Heuristik versucht der Agent seitlich und tangential auszuweichen, wenn der Weg zum Ziel versperrt ist. Sind auch die Aus\-weich\-mög\-lich\-kei\-ten versperrt, so wird versucht der nächsten Person in die gleiche Richtung zu folgen. Wenn jedoch kein Leader gefunden werden kann, oder ein Schritt in die Richtung von diesem zu einer Kollision führen würde, bleibt die Person stehen, sie verwendet die "`Step or wait"'-Heuristik. Der Algorithmus der "`aggressiven Follower"'-Heu\-ris\-tik ist in Algorithmus \ref{alg:aggressiveFollower} eingefügt.
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.55\textwidth]{pictures/model/algorithm/boarding/normal_boarding/normal_boarding_heuristic.png}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.7\textwidth]{pictures/model/algorithm/boarding/normal_boarding/normal_space_find_heuristic.png}
...
Das Prozessdiagramm der defensiver "`Einsteiger"'-Heuristik wird in \figurename \ref{fig:DEH} gezeigt.
Wie in dieser Abbildung zu sehen ist, überprüft ein defensiver Einsteiger zunächst, ob der letzte Aussteiger schon ausgestiegen ist. Ist der letzte Aussteiger aus der Tür, beginnt der defensive Einsteiger mit der "`Follower"'-Heuristik mit dem Einsteigen. Sind noch Aussteiger im Zug, überprüft der Agent seine aktuelle Position. Liegt diese im Wartebereich, verweilt er in seiner aktuellen Position. 
\begin{figure}[H]
	\centering
		\includegraphics[width=0.6\textwidth]{pictures/model/algorithm/boarding/defensive_boarding/defensive_boarding_heuristic.png}
	\caption{Prozessdiagramm "`defensiver Einsteiger"'-Heuristik. Quelle: Eigene Darstellung.}
	\label{fig:DEH}
\end{figure}
Ist der Einsteiger noch nicht im Wartebereich, sucht er mit Hilfe der "`defensiven Platzsuche"'-Heuristik einen Platz in diesem. Diese Heuristik kann in \figurename \ref{fig:DPH} betrachtet werden. Im Gegensatz zu normalen Einsteigern, wartet der defensive Einsteiger solange im Wartebereich, bis der letzte Aussteiger den Wagon verlässt, auch wenn genügend Platz ist. 
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.55\textwidth]{pictures/model/algorithm/boarding/defensive_boarding/defensive_space_find_heuristic.png}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.85\textwidth]{pictures/model/algorithm/boarding/aggressive_boarding/aggressive_space_find_heuristic.png}
...
Bei aggressiven Einsteigern wird das Ziel des Agenten auf einer der möglichen Positionen gesetzt, je nachdem welche frei ist. Mit der "`aggressiven Follower"'-Heu\-ris\-tik bewegt sich die Person dann zu diesem Platz.
...
Wie in \figurename \ref{fig:AH} zu erkennen, wird die Startzeit des Aussteigers vor dem Ausstiegsprozess festgelegt. Dabei wird diese durch Zufall aus dem entsprechenden Intervall ausgewählt. 
\begin{figure}[H]
	\centering
		\includegraphics[width=0.6\textwidth]{pictures/model/algorithm/alight/alight_process.png}
	\caption{Prozessdiagramm der Aussteiger: "`Ausstiegsprozess"'-Heuristik. Quelle: Eigene Darstellung.}
	\label{fig:AH}
\end{figure}Die Intervalle wurden hierbei durch die Auswertung in \ref{Startzeiten} ermittelt. Das Startzeitenintervall für defensive liegt zwischen $1.9$ und $2.1$ Sekunden. Für normale liegt es zwischen $1.6$ und $1.8$ Sekunden und für aggressive zwischen $1.3$ und $1.6$ Sekunden. Hierbei nehme ich an, dass die Werte im Intervall normal verteilt sind. Ist die Startzeit gewählt, so beginnt die "`Ausstiegsprozess"'-Heuristik. Dabei verbleibt die Person in ihrer aktuellen Position so lange nicht mehr Zeit seit der Türöffnung vergangen ist, als die Startzeit der Person. Wenn die Tür länger oder gleich der Startzeit geöffnet ist beginnt die Person mit der entsprechenden "`Follower"'-Heuristik den Ausstieg. Dabei wird die "`aggressive Follower"'-Heuristik für aggressive Typen verwendet. 
...
Im Algorithmus wird in jedem Schritt überprüft, ob seit der Türöffnung mehr Zeit vergangen ist als die Startzeit, ist das der Fall, verbleibt der Agent in seiner aktuellen Position, er wartet, bis die Türen weit genug geöffnet sind. Andernfalls steigt er mit Hilfe der Algorithmen für die "`Follower"'-Heuristik aus.

\subsection{Platzmacher-Heuristik}
Das Platzmacher-Modell wird durch die "`Platzmachprozess"'-Heuristik umgesetzt, die sowohl die "`Ausstiegsprozess"'-Heuristik, als auch die "`Einsteigeprozess"'-Heu\-ris\-tik verwendet.
\begin{figure}[H]
	\centering
		\includegraphics[width=0.48\textwidth]{pictures/model/algorithm/spacemaker/spacemaker_heuristic.png}
